    ...
    internal class OV : MappingBase
    {
        ...
        [
            new PatchEntry
            {
                ...
                {
                    {0x288ADF, [0xE9, 0x1D, 0xD2, 0x26, 0x00, 0x90, 0x90, 0x90]}, // High entity resolution
                    {0x4F5D01, [0xE8, 0x87, 0x3D, 0xD9, 0xFF, 0xC7, 0x46, 0x40, 0x00,
            ...
            new PatchEntry
            {
                ...
                Mapping = new Dictionary<long, byte[]>()
                {
                    {0x732FA, [0xC6, 0x05, 0x28, 0xBF, 0xAA, 0x00, 0x01, 0xEB, 0x7C, 0x90]}, // Set global DevMachine to 1
                    {0x0BE11, [0x66, 0x90]}, // Enable Development-Mode without command line argument -DevM
                }
            },
            new PatchEntry
            {
            ...
            new PatchEntry
            {
                ...
                Mapping = new Dictionary<long, byte[]>()
                {
                    {0x23D64D, [0x90, 0x90]}, // Override JZ, always set Special Edition to 1
                    {0x23D655, [0x90, 0x90]}, // Override JNZ, always set Special Edition to 1
                }
            },
        ];
        ...
        public override Dictionary<long, byte[]> GetEasyDebugMapping() => new()
        {
            ...
            {0x00BC4E, [0x90, 0x90]}, // Override JGE, show no data folder found MessageBox (halt thread)
            {0x00BCB5, [0xEB, 0x03, 0x90, 0x90, 0x90]}, // Jump to original instruction
        };
        ...
        public override Dictionary<long, byte[]> GetModloaderMapping() => new()
        {
            ...
            {0x23E916, [0xEB]}, // Always jump to ModelViewer
            {0x23E931, [0xE8, 0xA7, 0xF9, 0xFF, 0xFF]}, // Always jump to ModelViewer
            {0x23E936, [0xEB, 0xE0, 0x90, 0x90]}, // Return to original loader after mod
            // .text segment -> Override the ModelViewer func
            {0x23E236, [0x55, 0x89, 0xE5, 0x83, 0xEC, 0x40, 0x8D, 0x4D, 0xDC, 0x68, 0x00,
        ...
        public override Dictionary<long, byte[]> GetZoomLevelMapping(double ZoomLevel, float ClutterFarDistance) => new()
        {
            ...
            {0x2B334E, [0xC7, 0x45, 0x64]},
            {0x2B3351, BitConverter.GetBytes(ClutterFarDistance + 4800f)},
            {0x2B3355, [0xC7, 0x45, 0x6C]},
            {0x2B3358, BitConverter.GetBytes(3700f)},
            {0x2B335C, [0x90, 0x90]},
            {0x27AC99, [0x50, 0xB8, 0x00, 0x40, 0x9C, 0xC5, 0x89, 0x81, 0x9C,
