    ...
    internal class HEUbi : MappingBase
    {
        ...
        [
            new PatchEntry
            {
                ...
                {
                    {0x1E0837, [0xEB, 0x19]}, // Enable Development-Mode without command line argument -DevM
                    {0x204B57, [0xC6, 0x05]}, // Set global DevMachine to 1
                    {0x204B5D, [0x01]}, // Set global DevMachine to 1
                }
            },
            new PatchEntry
            {
            ...
            new PatchEntry
            {
                ...
                Mapping = new Dictionary<long, byte[]>()
                {
                    {0x25C6FA, [0x90, 0x90]}, // Override JZ, always set Special Edition to 1
                    {0x25C700, [0x90, 0x90, 0xFF, 0xC3]}, // Override JNZ, always set Special Edition to 1
                }
            },
        ];
        ...
        public override Dictionary<long, byte[]> GetEasyDebugMapping() => new()
        {
            ...
            {0x1E05CF, [0x90, 0x90]}, // Override JGE, show no data folder found MessageBox (halt thread)
            {0x1E064A, [0xEB, 0x03, 0x90, 0x90, 0x90]}, // Jump to original instruction
        };
        ...
        { 
            ...
            {0x25D526, [0xEB]}, // Always jump to ModelViewer
            {0x25D52A, [0xEB]}, // Always jump to ModelViewer
            {0x25D55A, [0xE8, 0x41, 0x00, 0x00, 0x00, 0xEB, 0xC7, 0x90]}, // Call ModelViewer method
            // .text segment -> Override the ModelViewer func
            {0x25D5CC, [0xD2]}, // Return to original loader after mod
            {0x25D5FA, [0xEB, 0x7D, 0x83, 0x7D, 0x20,
                0x01, 0x75, 0xB7, 0x68, 0xBA, 0x20, 0x05, 0x01,
                0xEB, 0xB5, 0x90]}, // Return to original loader after mod        
            {0x25D5AF, [0xEB, 0x4B, 0xE8, 0xA0, 0x12,
                0x9A, 0xFF, 0x90, 0x90, 0x90]}, // Return to original loader after mod
            {0x25D679, [0x90, 0x59, 0x59]}, // Return to original loader after mod
        };
        ...
        {
            {0xC4EC4C, BitConverter.GetBytes(ClutterFarDistance)}, // This is not an error, the HE uses the zoomlevel
                                                                   // as single instead of double like in the OV ...
            {0x270311, [0xC7, 0x45, 0xF0]},
            {0x270314, BitConverter.GetBytes(ClutterFarDistance + 4800f)},
            {0x270318, [0xC7, 0x45, 0xF4]},
            {0x27031B, BitConverter.GetBytes(3700f)},
            {0x27031F, [0x90]},
            {0x270325, [0x90, 0x90, 0x90]},
            {0x2532F7, [0x50, 0xB8, 0x00, 0x40, 0x9C, 0xC5, 0x89, 0x81,
